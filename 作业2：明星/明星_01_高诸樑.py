...
def create_database():
    """创建数据库"""
    dbpath = r'C:\Users\Admin\star7.sqlite'
...
def get_height(html):
    ...
    if re.search(
            r'<dt class="basicInfo-item name">身&nbsp;&nbsp;&nbsp;&nbsp;高</dt>\n<dd class="basicInfo-item value">([\d\D]*?)<',
            html):
        height = re.search(
            r'<dt class="basicInfo-item name">身&nbsp;&nbsp;&nbsp;&nbsp;高</dt>\n<dd class="basicInfo-item value">([\d\D]*?)<',
            html).group(1).strip()
...
def get_xz(html):
    ...
    if re.search(
            r'<dt class="basicInfo-item name">星&nbsp;&nbsp;&nbsp;&nbsp;座</dt>\n<dd class="basicInfo-item value">\n<a[\d\D]*?>([\d\D]*?)</a',
            html):
        xingzuo = re.search(
            r'<dt class="basicInfo-item name">星&nbsp;&nbsp;&nbsp;&nbsp;座</dt>\n<dd class="basicInfo-item value">\n<a[\d\D]*?>([\d\D]*?)</a',
            html).group(1).strip()
        ...
        db.commit()
            ...
            html):
        xingzuo = re.search(
            r'<dt class="basicInfo-item name">星&nbsp;&nbsp;&nbsp;&nbsp;座</dt>\n<dd class="basicInfo-item value">([\d\D]*?)</dd',
            html).group(1).strip()
        s.xz = xingzuo
        db.commit()
...
def get_address(html):
    ...
    if re.search(
            r'<dt class="basicInfo-item name">出生地</dt>\n<dd class="basicInfo-item value">\n<a [\d\D]*?>([\d\D]*?)</a',
            html):
        jiguan = re.search(
            r'<dt class="basicInfo-item name">出生地</dt>\n<dd class="basicInfo-item value">\n<a [\d\D]*?>([\d\D]*?)</a',
            html).group(1).strip()
